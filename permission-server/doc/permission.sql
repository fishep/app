...
(
    ...
    `guard`      varchar(15)  not null comment '所属系统, 用guard来表示',
    `is_system`  tinyint      not null default 0 comment '是否系统角色，0：否，1：是，系统角色不允许删除',
...
(
    ...
    `is_default` tinyint     not null default 0 comment '是否是用户默认的角色，0：否，1：是，当用户有多个角色，需要选出一个时使用',
    `created_at` bigint      not null default 0 comment '创建时间',
...
values (1, 'SuperAdmin', 'ERP', 1, '{"en_US": "SuperAdmin", "zh_CN": "超管"}', '系统角色，超级管理员，拥有所有的权限', 1681987720955);
...
insert into user_roles(`user_type`, `user_id`, `role_id`, `is_default`, `created_at`) values ('ADMIN', 1, 1, 1, 1681987720955);
